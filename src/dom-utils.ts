...
function createElement(tagName: string, properties?: any): HTMLElement {
    var newElement: { [key: string]: any } = document.createElement(tagName);
    if (properties)
        Object.keys(properties).forEach(key => newElement[key] = <any>properties[key]);
