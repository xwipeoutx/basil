...
function hookNodeListeners() {
    var depths = {};
    var depth = 1;
    var loadingIndicators = ['|', '/', '-', '\\'];
    var loadingIndex = 0;
    basil.events.nodeEntered.subscribe(function (test) {
    ...
    basil.events.nodeExited.subscribe(function (test) { return depth--; });
    basil.events.rootStarted.subscribe(function (test) { return console.log(spaces() + test.name); });
    basil.events.rootComplete.subscribe(function (test) {
        console.log(col.move(0, -2));
        writeStatus(test, depth);
    });
    function writeStatus(test, depth) {
...
exports.hookNodeListeners = hookNodeListeners;
